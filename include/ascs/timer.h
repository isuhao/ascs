...
public:
	...
	void stop_all_timer(tid excepted_id) {do_something_to_all([=](auto& item) {if (excepted_id != item.id) this->stop_timer(item);});}
...
protected:
	void start_timer(timer_info& ti)
	{
		...
		ti.timer->async_wait(make_handler_error([this, &ti](const auto& ec) {
